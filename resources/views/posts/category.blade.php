<x-app-layout>
    ...
    <x-slot name='header'>
        ...
        @if($tag)
            @if($tag->position==1)
                <span class="uppercase text-lg bg-gray-50 p-0.5 border-gray-500 border rounded-lg text-gray-700 font-medium">{{$tag->name}}</span>
            @endif
            @if($tag->position==2)
                <span class="uppercase text-lg bg-blue-50 p-0.5 border-blue-500 border rounded-lg text-blue-700 font-medium">{{$tag->name}}</span>
            @endif
            @if($tag->position==3)
                <span class="uppercase text-lg bg-green-50 p-0.5 border-green-500 border rounded-lg text-green-700 font-medium">{{$tag->name}}</span>
            @endif
        @endif
        </div>
    </x-slot>
