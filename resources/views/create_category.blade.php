<x-app-layout>
    ...
    <x-slot name='header'>
        ...
        <a class="text-xl" href={{route('users.show',Auth::user()->name)}}>{{Auth::user()->name}}</a>
        ...
        <span class="text-xl">新規カテゴリ作成</span>
        </div>
    </x-slot>
    <div class='flex-1 bg-gray-200 p-4'>
        <form action='/categories' method='POST' enctype='multipart/form-data'>
            ...
            <div class="mt-12 mb-8">
                ...
                <input id="title" type="text" name="category[name]" class="border-0 outline-none mx-2 mt-2 placeholder-gray-500 placeholder-opacity-50 w-full py-2" placeholder="カテゴリ名を入力">
            </div>
            <div class="mb-4">
                <span for="tag_name_0" class="ml-2 text-sm w-fit block bg-gray-50 p-0.5 border-gray-500 border rounded text-gray-700 font-medium">ステータス名1</span>
                <input id="tag_name_0" type="text" name="tag[0][name]" class="border-0 outline-none mx-2 mt-2 placeholder-gray-500 placeholder-opacity-50 w-1/4 py-2" value="未着手" placeholder="タグ名を入力">
            </div>
            <div class="mb-4">
                <span for="tag_name_1" class="ml-2 text-sm w-fit block bg-blue-50 p-0.5 border-blue-500 border rounded text-blue-700 font-medium">ステータス名2</span>
                <input id="tag_name_1" type="text" name="tag[1][name]" class="border-0 outline-none mx-2 mt-2 placeholder-gray-500 placeholder-opacity-50 w-1/4 py-2" value="進行中" placeholder="タグ名を入力">
            </div>
            <div class="mb-8">
                <span for="tag_name_2" class="ml-2 text-sm w-fit block bg-green-50 p-0.5 border-green-500 border rounded text-green-700 font-medium">ステータス名3</span>
                <input id="tag_name_2" type="text" name="tag[2][name]" class="border-0 outline-none mx-2 mt-2 placeholder-gray-500 placeholder-opacity-50 w-1/4 py-2" value="完了" placeholder="タグ名を入力">
            </div>
            <div class="mb-8">
                <label for="body" class="ml-2 text-sm block">カテゴリ説明</label>
                <textarea id="body" name="category[category_explanation]"  class="h-40 border-0 outline-none mx-2 mt-2 placeholder-gray-500 placeholder-opacity-50 w-full py-2" placeholder="カテゴリ説明を入力"></textarea>
            </div>
            <div class="flex justify-end mt-4">
                <input type='submit' value='作成' class="px-4 py-2 bg-green-600 text-white rounded">
            </div>
        </form>
    </div>
</x-app-layout>
